...
sections = {
    "What is Python":"Python is a versatile, high-level, general-purpose programming language known for its readability and simplicity. It's widely used in various applications like web development, data science, and machine learning. Python's design emphasizes code clarity and utilizes indentation to structure code blocks. It supports multiple programming paradigms, including object-oriented, functional, and procedural programming. ",
    "🔤 Variables & Data Types": "Variables in Python are symbolic names that are used to store data values. They act as containers for holding information that can be referenced and manipulated throughout a program. Unlike some other programming languages, Python does not require explicit declaration of variable types; the type is inferred at runtime based on the assigned value.\n\nPython has several built-in data types that can be broadly categorized as follows:\nint,float,complex etc." ,
    "🔁 Loops": "Loops in Python facilitate the repeated execution of a block of code. Python supports two primary types of loops: for loops and while loops.\n\nFor loop: A for loop iterates over a sequence (like a list, tuple, or string) or other iterable objects. It executes the loop body for each item in the sequence.\n\nWhile loop: A while loop executes a block of code repeatedly as long as a condition remains true.",
    "📦 Functions": "A function in Python is a block of organized, reusable code that performs a specific task. Functions help break down larger programs into smaller, manageable chunks, improving code readability, reusability, and maintainability.",
    "📄 File Handling": "File handling in Python enables programs to interact with files stored on a computer's storage devices. This allows for data to be read from and written to files, making it possible to persist data between program executions. The process involves opening a file, performing operations like reading or writing, and then closing the file.\n\nOpening a File: The open() function is used to open a file. It takes two arguments: the file name and the mode. The mode specifies the operation to be performed on the file, such as reading ('r'), writing ('w'), appending ('a'), or both reading and writing ('r+').",
    "🧪 Exception Handling": "Exception handling in Python is a mechanism to gracefully manage errors that occur during the execution of a program. It prevents the program from crashing and allows it to continue running or terminate in a controlled manner. The core components of exception handling in Python are the try, except, else, and finally blocks.\ntry:  This block encloses the code that might raise an exception.\nexcept:  If an exception occurs within the try block, the code execution immediately jumps to the except block that matches the type of exception raised.\nelse:  This block is optional and is executed only if no exception occurs in the try block.\nfinally:  This block is also optional and is always executed, regardless of whether an exception occurred or not. It is typically used for cleanup actions."
}
